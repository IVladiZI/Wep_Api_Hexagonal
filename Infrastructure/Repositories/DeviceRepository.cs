    ...
    public class DeviceRepository : IDevicePersistence
    {
        ...
        public async Task<long> InsertDeviceAsync(string deviceId, string deviceType)
        {
            try
            {
                ...
                using var command = new SqlCommand("bill.DISPOSITIVO_InsertMod", connection)
                {
        ...
        public async Task<long> GetIdDevice(string deviceId,string deviceType)
        {
            try
            {
                using var connection = new SqlConnection(_connectionString);
                using var command = new SqlCommand("bill.DISPOSITIVO_GetByIdTypeMod", connection)
                {
                    CommandType = CommandType.StoredProcedure
                };
